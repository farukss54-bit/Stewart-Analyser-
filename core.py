...
def calculate_be(ph: float, hco3: float) -> float:
    """
    Siggaard-Andersen denklemi ile Base Excess hesaplama.

    Formül: BE = 0.9287 × (HCO3 - 24.4 + 14.83 × (pH - 7.4))

    Bu formül, Van Slyke yaklaşımına göre klinik pratikte daha yaygın
    kullanılır ve referans laboratuvar cihazlarında standart olarak
    uygulanmaktadır.

    Referans: Siggaard-Andersen equation (klinik standart)

    Test: pH=7.40, HCO3=24 → BE≈0
    """
    return round(BE_COEFFICIENT * (hco3 - BE_HCO3_NORMAL + BE_PH_COEFFICIENT * (ph - BE_PH_NORMAL)), 1)
